...
- 晚间推送附带 DDL 提醒与任务回顾（#91）：`generate_tomorrow_plan` 在**预览次日**时于 `preview` 末尾追加「———— 晚间小结 ————」——`📌 明天有 N 个 DDL：…`（口径：`tasks.deadline == 次日` 且状态 `todo`/`doing`）＋`❓ 今天安排的任务都完成了吗？`并列出当天 `plan_items` 中 `task`/`review`/`misc` 里状态非 `done`/`skipped` 的项；`get_today_plan_preview` 不带小结，09:00 晨间推送输出与旧版完全一致
- 预览标题与操作提示按日期区分（#91）：21:00 推的是次日计划，此前一律显示「今日计划」易误读；现标题按日期输出「明日计划 / 今日计划」，结尾提示同步区分
- 每日 18:10-18:40「英语单词」写入用户画像 `fixed_activities`（#91），规划器自动视其为屏障，不再往该时段排任务
- pytest 新增 5 例（日期标签/提示、晚间小结含与不含 DDL、`evening_digest` 默认关闭），全量 297 例通过
- 新增 `update_task` MCP 工具（#83）：修改已有任务的标题/截止日期/类型/预估时长/状态/所属课程，本地与 Notion 任务库同步更新（尽力而为）；NotionTaskWriter 新增 update_task/set_status；新增 6 例测试

### Changed

- 用户画像固定安排调整（#91）：移除周一「打扫宿舍卫生」，由每日「英语单词」（18:10-18:40）接替该时段；`fixed_activities` 仍为 10 项
- 课程档位制调整（#74）：**废除 C 档**（S/A/B 三档，旧库 CHECK 约束自动重建 + C→B 数据迁移）；**S 档课程课后自动安排 1 小时复习**（`generate_plan` 注入 `复习 · <课程>（课后）`，规划器新增 `not_before` 保证紧排课后、只推后不提前）；**B 档课程照常占住日程**，标题标注「（可做别的事，效率减半）」（不再走 release_slot 释放时段，该字段转为预留）；数据迁移：数据库系统实验→A、航空航天材料工程→S、删除已退课的钙钛矿材料及其柔性光电子器件（随约束重建级联删除时间块/知识点/复习计划）；pytest 新增 5 例，全量 326 例通过
- 兜底定时任务（每晚 21:00）升级 auto_confirm：生成次日计划后直接确认并写入 Notion 日历，不再依赖 WorkBuddy 触发确认（连续两晚因确认环节断链致日历无写入，#72）；Notion 异常不阻断本地确认。新增 3 例调度器测试；修正 2 例跑量计划测试的日期窗错配（end_date 未锚定）

### Fixed

- 文档残留清理（#91 / #86 后续）：`docs/mcp-server.md` 第 6 节「后端 21:00 兜底（APScheduler）」改为「后端不定时」（v3 起 `scheduler_jobs.py` 已删除，全部定时触发归 WorkBuddy）；架构图去掉 `scheduler_jobs.py` 行；「启动日志会出现 MCP 定时任务已启动」删除；环境变量表去掉已作废的 `JREN_MCP_SCHEDULER_ENABLED` / `JREN_MCP_PLAN_GENERATE_TIME`；两处「防火墙放行 8000」更正为 28070
- 文档与 v3 实际状态对齐（#86）：定时任务时间统一为 **21:00 / 09:00**（原文写 08:20，与
  WorkBuddy 自动化实际配置不符）；MCP 工具数统一为 **12**（原文 11/13 两种说法并存，
  且工具表缺 #83 新增的 `update_task`）；清理 #80 已删功能的文档残留——移除
  `get_running_data` / `generate_running_plan` 工具行、「5.1 跑步训练计划（高驰）」
  整节、以及「计划生成读 Obsidian 笔记」「21:00 生成由后端服务负责」等过时描述；
  微信通道实测记录补充 `ret=-2 prepare failed` 的 token 失效处置。措辞「08:00 推送」统一为「晨间推送」
- 后端端口 8001 → 28070：雷神加速器组件 mrender.exe 开机随机绑定 1.1 万+ 端口（1025~14994 撒网），8000/8001 均在活动区；迁移至高位冷门端口一劳永逸（#70）
