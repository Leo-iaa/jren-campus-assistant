...
def start_scheduler_if_enabled() -> BackgroundScheduler | None:
    ...
    _scheduler.add_job(
        ...
        replace_existing=True,
        # 进程存活但错过触发点（休眠唤醒等）时 1 小时内补跑；
        # 注意：进程重启后 APScheduler 不会补跑 misfire，重启场景由
        # run_startup_catchup 的晨间补偿分支负责
        misfire_grace_time=3600,
    )
    _scheduler.add_job(
        evening_push_job,
...
def run_startup_catchup(now: datetime | None = None, *, _skip_morning_push: bool = False) -> None:
    ...
    try:
        ...
        # morning_push_job 幂等（当日标记防重）；_skip_catchup=True 防止
        # 与本函数互相递归（今日计划缺口已在上面 a) 分支补齐）
...
def _push_text_via_cli(text: str) -> tuple[bool, str]:
    ...
    if (
        "失效" in detail
        or "尚未获取 token" in detail
        or "TOKEN_EXPIRED" in detail
        or "ret -2" in detail
        or "errcode -14" in detail
    ):
        detail += (
...
def morning_push_job(now: datetime | None = None, *, _skip_catchup: bool = False) -> None:
    ...
    2) 复用 run_startup_catchup 确保今日计划存在（21:00 前补生成 + 幂等确认）；
       ``_skip_catchup=True`` 仅供 run_startup_catchup 的晨间补偿分支使用，
       防止互相递归（计划缺口已由调用方补齐）
