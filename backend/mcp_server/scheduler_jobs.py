"""APScheduler 定时任务：生成计划 + 微信推送（后端兜底通道）。

设计依据 docs/vision.md「提醒链路（方案 A）」：

- 21:00 生成次日计划并**自动确认写入 Notion 日历**（auto_confirm，
  2026-08-31 起：原草案模式连续两晚因确认环节断链导致日历无写入）
- 09:35 晨间推送 / 21:05 晚间推送（实现拆在 :mod:`wechat_push`，
  后端兜底取代不可靠的 WorkBuddy 应用内调度器）
- 开机 / 重启后 :func:`run_startup_catchup` 补齐关机休眠错过的缺口

- 触发时间：``JREN_MCP_PLAN_GENERATE_TIME`` / ``JREN_MCP_MORNING_PUSH_TIME``
  / ``JREN_MCP_EVENING_PUSH_TIME``（默认 21:00 / 09:35 / 21:05，Asia/Shanghai）
...
def generate_tomorrow_plan_job() -> None:
    ...
    from backend.mcp_server._common import tomorrow
    from backend.mcp_server.plan_generator import generate_plan
    from backend.mcp_server.plan_lifecycle import confirm_plan
...
def run_startup_catchup(now: datetime | None = None, *, _skip_morning_push: bool = False) -> None:
    ...
    - 已过晚间推送点且当天未推过 → 补推次日计划（evening_push_job 幂等）
    - 已过晨间推送点、未到晚间推送点且当天未推过 → 补推今日计划
      （morning_push_job 幂等；``_skip_morning_push=True`` 供 morning_push_job
      自身调用时跳过本分支，防止自我递归重复推送）
    ...
    from backend.mcp_server._common import tomorrow as service_tomorrow
    from backend.mcp_server.plan_generator import generate_plan
    from backend.mcp_server.plan_lifecycle import confirm_plan
    from backend.mcp_server.wechat_push import evening_push_job, morning_push_job
    ...
    try:
        ...
        # 晨间推送补偿：已过晨间推送点、未到晚间推送点（其后由晚间推送覆盖，
        # 避免重启时早晚连发两条）且当天未推过 → 补推今日计划。
        # _skip_catchup=True 防止与本函数互相递归（今日计划缺口已在 a) 分支补齐）
        if not _skip_morning_push:
            morning_due = _parse_hhmm(
                settings.mcp_morning_push_time,
                default=(9, 35),
                label="mcp_morning_push_time",
            )
            if morning_due <= (now.hour, now.minute) < evening_due:
                morning_push_job(now, _skip_catchup=True)
...
def start_scheduler_if_enabled() -> BackgroundScheduler | None:
    ...
    _scheduler.add_job(
        ...
        replace_existing=True,
        # 进程存活但错过触发点（休眠唤醒等）时 1 小时内补跑；进程重启后
        # APScheduler 不补跑 misfire，重启场景由 run_startup_catchup 补偿
        misfire_grace_time=3600,
    )
    _scheduler.add_job(
        _evening_push_entry,
        CronTrigger(hour=evening_hour, minute=evening_minute, timezone="Asia/Shanghai"),
    ...
    logger.info(
        ...
        hour, minute, push_hour, push_minute, evening_hour, evening_minute,
    )
    return _scheduler


def _morning_push_entry() -> None:
    """调度器入口：延迟导入避免 wechat_push ↔ scheduler_jobs 循环依赖。"""
    from backend.mcp_server.wechat_push import morning_push_job

    morning_push_job()


def _evening_push_entry() -> None:
    from backend.mcp_server.wechat_push import evening_push_job

    evening_push_job()


# 兼容导出：测试 / 外部调用仍按 scheduler_jobs.morning_push_job 访问
# （实现与标记常量在 wechat_push 模块；job id 常量留在这里，归调度器管）
from backend.mcp_server.wechat_push import (  # noqa: E402,F401
    EVENING_PUSH_MARK_KEY,
    MORNING_PUSH_MARK_KEY,
    evening_push_job,
    morning_push_job,
    push_text_via_cli,
)
