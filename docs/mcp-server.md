...
**自查服务是否在跑（大白话）**：浏览器打开 `http://127.0.0.1:28070/health`，
...
**计划联动设计理由**（无确认概念，Issue #58）：日常节奏 = 每晚 21:00 `auto_confirm=true`
生成并确认次日计划写日历；之后用户随时加任务，**直接增量插入**目标日空闲时段——
**不动已排好的其它安排**，也不存在「已确认就不让改」的锁：

| 任务 ddl | 自动行为 |
|----------|----------|
| 无 ddl（「今天还有什么任务」） | 直接插入**今天**空闲时段（最早适配，8:00-22:00） |
| ddl = 今天 | 直接插入**今天** |
| ddl = 明天 | 直接插入**明天**（不占用今天） |
| ddl 更远 | 不占位，下次 21:00 生成次日计划时自动纳入并确认 |
| 已过期 | 不自动排，提示手动处理 |

- **增量插入**：找目标日首个空闲时段安插（避开课程与已有计划项），已有安排一个不动；
  插入项状态与当日一致（当日已确认 → confirmed），并**同步更新 Notion 日历**
- **排满时按 ddl 动态调整**（Issue #58）：目标日无空闲时段且新任务带 ddl（急事）时，
  自动找出目标日里 **ddl 晚于新任务**的已排任务，按 ddl 从晚到早**顺延到它自己的 ddl 当天**
  （ddl 当天有空档才挪），腾出空间插入急事——例如「今天排满 + 今晚急事」会把
  「明天截止」的任务挪到明天、急事排今天。只动 task 类型项，课程/复习/杂项永不挪；
  顺延记录在返回的 `evicted` 字段，message 里逐一说明
- 目标日排满且没有可顺延的任务 → 明确提示「排不下了」+ 引导「把 XX 挪到 HH:MM」手动安排
- 用户无需确认：21:00 自动确认写日历 + 临时插入直接生效
