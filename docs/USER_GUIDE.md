...
```text
第一步：双击 setup.bat 一键安装        （约 5 分钟，自动完成依赖+数据库）
...
1. 先确认 Python 是真的装好了（检查方法见下方「第 0 步」）
2. 双击仓库根目录的 **`setup.bat`**
3. 脚本自动完成：检查 Python → 创建虚拟环境 → 安装依赖 → 初始化数据库
4. 看到 `Setup complete!` 即安装完成，直接跳到 [4.5 启动后端](#45-启动后端)
