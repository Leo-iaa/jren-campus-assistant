...
2. 验证是否成功：浏览器打开 `http://127.0.0.1:28070/health`
3. 看到 `{"status":"ok",...}` 就是成功了 ✅

> 接口页面（可看可点）：浏览器打开 `http://127.0.0.1:28070/docs`
...
**服务在不在跑？** 浏览器打开 `http://127.0.0.1:28070/health`，能看到 ok 就是在跑。
...
> 导入后每门课默认为 **A 档**；课程档位（S/A/B）可在设置页或对话中调整。
...
**Q：`http://127.0.0.1:28070/health` 打不开？**
A：服务没在运行。双击 `backend\scripts\start_backend.bat` 手动启动；如果还不行，重启电脑让开机自启生效。

**Q：WorkBuddy 提示连不上 MCP？**
A：① 先确认 `http://127.0.0.1:28070/health` 能打开（服务在跑）；② 检查 MCP 地址是否 `http://127.0.0.1:28070/mcp`、类型是否 **http**；③ WorkBuddy 装在其他设备时改用 `http://<电脑IP>:28070/mcp` 并确认防火墙放行。
